<template>
  <div class="content">
    <div class="top-wrap">
      <div class="top">
        <div class="left">
          <ImageCompare style="width: 520px; height: 520px" before="https://jyzn-prod.oss-cn-shanghai.aliyuncs.com/images/colorize-banner01-before.jpg" after="https://jyzn-prod.oss-cn-shanghai.aliyuncs.com/images/colorize-banner01-after.jpg" />
        </div>
        <div class="right">
          <div class="title">图片上色</div>
          <div class="description">利用AI技术对照片上色，一键黑白转彩色。适合人物，风景，纪实等各类风格。保存回忆，情景重现。</div>
          <div class="btn-group">
    ...
    <div ref="imagesRef" v-show="imageInfo.progress" class="images">
      ...
      <div class="images-content">
        ...
        <div class="images-content-right">
          ...
          <div class="img-wrap">
            <img v-if="imageInfo.colorizeUrl" :src="imageInfo.colorizeUrl" alt="" />
...
<script lang="ts">
  ...
  import { getUploadSingnatureApi, getUploadKeyApi, getUploadFileApi, colorizeApi, uploadImageByUrlApi, getDictDataByTypeApi, downloadUrlApi } from '/@/api/api';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const process = () => {
        ...
        imageInfo.colorizeUrl = '';
        colorizeApi(imageInfo.id).then((res) => {
          imageInfo.colorizeUrl = res.data.url;
          imageInfo.id = res.data.id;
        });
      };
      ...
      const currentHost = window.location.origin + '/mobileUpload?type=colorize&authorization=' + userStore.getToken + '&key=';

      const handleDownloadImg = () => {
        if (!imageInfo.colorizeUrl) {
      ...
      onBeforeMount(() => {
        getDictDataByTypeApi('example_colorize').then((res) => {
          if (res.code == 200) {
