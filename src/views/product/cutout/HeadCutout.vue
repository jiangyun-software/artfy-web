  ...
  <div class="content">
    <div class="top-wrap">
      <div class="top">
        ...
        <div class="right">
          <div class="title">头部抠图</div>
          <div class="description">自动识别上传图像中的头部轮廓，可适用于多人场景， 单色背景与头部轮廓明显的图像抠图效果会更好。</div>
          <div class="btn-group">
...
<script lang="ts">
  ...
  import { getUploadSingnatureApi, getUploadKeyApi, getUploadFileApi, headCutoutApi, uploadImageByUrlApi, getDictDataByTypeApi, downloadUrlApi } from '/@/api/api';
    ...
    setup() {
      ...
      const uploadAndCutout = (data) => {
        getUploadSingnatureApi('universalCutout').then((res) => {
          const image = { url: res.data.url, progress: 0, done: false };
          ...
          axios({
            ...
            headCutoutApi(res.data.id).then((res3) => {
              const img = imgList.value.find((item) => item.url == res.data.url);
              img.url = res3.data[0].url;
              img.done = true;
              img.id = res3.data[0].id;
            });
          });
        });
      };
      ...
      const tryExample = (url) => {
        cutoutWrapRef.value.scrollIntoView({ behavior: 'smooth', block: 'start' });
        headCutoutApi(url).then((res) => {
          if (res.code == 200) {
            const image = { url: res.data[0].url, progress: 100, done: true, id: res.data[0].id };
            imgList.value.push(image);
          }
        });
      };
        ...
        while (times > 0) {
          ...
          if (result.data.status == '0') {
            ...
            fileList.forEach((item) => {
              cutoutWrapRef.value.scrollIntoView({ behavior: 'smooth', block: 'start' });
              headCutoutApi(item.id).then((res) => {
                const img = imgList.value.find((item2) => item2.url == item.url);
                img.url = res.data[0].url;
                img.done = true;
                img.id = res.data[0].id;
              });
            });
      ...
      const handlePaste = (e) => {
        ...
        if (paste.files && paste.files.length > 0 && paste.files[0].type === 'image/png') {
          ...
          uploadImageByUrlApi(text).then((res) => {
            cutoutWrapRef.value.scrollIntoView({ behavior: 'smooth', block: 'start' });
            headCutoutApi(res.data).then((res3) => {
              const img = { url: res3.data[0], progress: 100, done: true };
      ...
      const currentHost = window.location.origin + '/mobileUpload?type=headCutout&authorization=' + userStore.getToken + '&key=';
      ...
      const banner = ['https://jyzn-prod.oss-cn-shanghai.aliyuncs.com/images/headCutout-banner01-raw.png', 'https://jyzn-prod.oss-cn-shanghai.aliyuncs.com/images/headCutout-banner01-cutout.png'];
      ...
      onBeforeMount(() => {
        getDictDataByTypeApi('example_headCutout').then((res) => {
          if (res.code == 200) {
