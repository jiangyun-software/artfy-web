  ...
  <div class="content">
    <div class="top-wrap">
      <div class="top">
        <div class="left">
          <div class="banner">
            <transition v-for="(item, index) in banner" :key="item" name="fade">
              <img v-show="index == bannerIndex" :src="item" alt="" />
            </transition>
          </div>
        </div>
        <div class="right">
          <div class="title">通用抠图</div>
          <div class="description">抠图从未如此简单！人物，商品，动物全部一键搞定。精确识别主体并移除背景，并支持自定义更换背景颜色。</div>
          <div class="btn-group">
          ...
          <div class="try-content">
            ...
            <div class="try">
              <img v-for="item in example" :key="item.label" class="try-item" :src="item.value" alt="" @click="tryExample(item.label)" />
            </div>
          </div>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    components: { Button, Input, CaretDownFilled, PlusOutlined, Progress, QrcodeVue, Modal, DeleteOutlined, DownloadOutlined },
    setup() {
      ...
      const uploadAndCutout = (data) => {
        getUploadSingnatureApi('universalCutout').then((res) => {
          ...
          axios({
            ...
            cutoutApi({ url: res.data.id }).then((res3) => {
      ...
      const tryExample = (url) => {
        cutoutWrapRef.value.scrollIntoView({ behavior: 'smooth', block: 'start' });
        cutoutApi({ url: url }).then((res) => {
          if (res.code == 200) {
        ...
        while (times > 0) {
          ...
          if (result.data.status == '0') {
            ...
            fileList.forEach((item) => {
              cutoutWrapRef.value.scrollIntoView({ behavior: 'smooth', block: 'start' });
              cutoutApi({ url: item.id }).then((res) => {
      ...
      const handlePaste = (e) => {
        ...
        if (paste.files && paste.files.length > 0 && paste.files[0].type === 'image/png') {
          ...
          uploadImageByUrlApi(text).then((res) => {
            cutoutWrapRef.value.scrollIntoView({ behavior: 'smooth', block: 'start' });
            cutoutApi({ url: res.data.id }).then((res3) => {
              const img = { url: res3.data, progress: 100, done: true, id: res3.data.id };
      ...
      const currentHost = window.location.origin + '/mobileUpload?type=universalCutout&authorization=' + userStore.getToken + '&key=';
      ...
      const banner = [
        'https://jyzn-prod.oss-cn-shanghai.aliyuncs.com/images/universalCutout-banner01-raw.png',
        'https://jyzn-prod.oss-cn-shanghai.aliyuncs.com/images/universalCutout-banner01-cutout.png',
        'https://jyzn-prod.oss-cn-shanghai.aliyuncs.com/images/universalCutout-banner02-raw.png',
        'https://jyzn-prod.oss-cn-shanghai.aliyuncs.com/images/universalCutout-banner02-cutout.png',
      ];
      ...
      onBeforeMount(() => {
        getDictDataByTypeApi('example_universalCutout').then((res) => {
          if (res.code == 200) {
