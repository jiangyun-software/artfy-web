  ...
  <div class="content">
    <div class="top-wrap">
      <div class="top">
        ...
        <div class="right">
          <div class="title">人像抠图</div>
          <div class="description">自动识别图像中的人体轮廓，发丝级专业抠图，一键切换背景颜色，秒变经典艺术照。</div>
          <div class="btn-group">
        ...
        <div class="tools">
          <div class="background">
            ...
            <div class="color-list">
              ...
              <div v-for="item in colorList" :key="item" class="color-item" @click="setBackgroundColor(item)" :style="{ background: item }"></div>
            </div>
          </div>
          <div class="crops">
...
<script lang="ts">
  import { defineComponent, reactive, ref, computed, onBeforeMount, onMounted } from 'vue';
  ...
  import { getUploadSingnatureApi, getUploadKeyApi, getUploadFileApi, cutoutApi, getDictDataByTypeApi, downloadUrlApi } from '/@/api/api';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const uploadAndCutout = (data) => {
        getUploadSingnatureApi('portraitCutout').then((res) => {
          ...
          axios({
            ...
            cutoutApi({ url: res.data.id, type: 'person' }).then((res3) => {
      ...
      const tryExample = (item) => {
        cutoutWrapRef.value.scrollIntoView({ behavior: 'smooth', block: 'start' });
        cutoutApi({ url: item.label, type: 'person' }).then((res) => {
          if (res.code == 200) {
        ...
        while (times > 0) {
          ...
          if (result.data.status == '0') {
            ...
            fileList.forEach((item) => {
              cutoutWrapRef.value.scrollIntoView({ behavior: 'smooth', block: 'start' });
              cutoutApi({ url: item.id, type: 'person' }).then((res) => {
      ...
      const handlePaste = (e) => {
        ...
        if (paste.files && paste.files.length > 0 && paste.files[0].type === 'image/png') {
          ...
          imgList.value.push({ url: text, progress: 100 });
        } else {
      ...
      const currentHost = window.location.origin + '/mobileUpload?type=portraitCutout&authorization=' + userStore.getToken + '&key=';
      ...
      const banner = ['https://jyzn-prod.oss-cn-shanghai.aliyuncs.com/images/portraitCutout-banner01-raw.png', 'https://jyzn-prod.oss-cn-shanghai.aliyuncs.com/images/portraitCutout-banner01-cutout.png'];
      ...
      onBeforeMount(() => {
        getDictDataByTypeApi('example_portraitCutout').then((res) => {
          if (res.code == 200) {
